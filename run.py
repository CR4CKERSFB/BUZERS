...
def banner():
	clear()
	wel='# [1] CRACK FILE CLONING (FREE)'
	cik2=mark(wel ,style='cyan')
	sol().print(cik2)
	ban='''


    dMP dMP dMP dMMMMb  .aMMMMP dMMMMMP dMMMMb 
   dMP.dMP amr dMP dMP dMP"    dMP     dMP"dMP 
  dMMMMK" dMP dMP dMP dMP MMP"dMMMP   dMMMMK"  
 dMP"AMF dMP dMP dMP dMP.dMP dMP     dMP.aMF   
dMP dMP dMP dMP dMP  VMMMP" dMP     dMMMMP"    
                                               

 
88  dP 88 88b 88  dP""b8 888888 88""Yb 
88odP  88 88Yb88 dP   `" 88__   88__dP 
88"Yb  88 88 Y88 Yb  "88 88""   88""Yb 
88  Yb 88 88  Y8  YboodP 88     88oodP 
─━㋱ STATUS : PREMIUM WHATSAPP : 0819-0776-1235 ㋱━─
─━㋱ STATUS : PRIBADI WHATSAPP : 0819-0776-1235 ㋱━─'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold cyan]•[CRACK DULU KONTOL]•[/bold cyan]'))
# VALIDASI TOKEN
def login():
...
def login_lagi334():
	banner()
	sky = '[bold cyan][>_] Version : 1.0 ✓\n[2] LOGIN COOKIE V2 KONTOL[/bold cyan]'
	sky2 = nel(sky, style='cyan')
	cetak(nel(sky2,title='[bold cyan] •[ CRACK NOMOR BERAPA ]• [/bold cyan]'))
	pil=input('    [P] PILIH KONTOL : ')
	if pil in ['1','01']:
		try:
...
def result():
	...
	kayes = '[bold cyan][C] CHECK CP RESULTS\n[C] CHECK OK RESULTS\n[0] BACK TO MENU[/bold cyan]'
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/CHANG/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/CHANG/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/CHANG/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/CHANG/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/CHANG/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/CHANG/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/CHANG/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/CHANG/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/CHANG/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def setting():
	...
	ioz = '[1] METHOD M-FACEBOOK\n[2] METHOD FREE-FACEBOOK\n[3] METHOD TOUCH-FACEBOOK\n[4] METHOD MBASIC-FACEBOOK'
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/CHANG-FB/CP')
	except:pass
	try:os.mkdir('/sdcard/CHANG-FB/OK')
	except:pass
	try:os.mkdir('/sdcard/CHANG-FB/DUMP')
	except:pass
	login()
